...
protected:
    void SetUp() override {
        ...
        handler_ = std::make_unique<GetOrderHandler>(mockBroker_);
    }
...
TEST_F(GetOrderHandlerTest, Found_Returns200) {
    auto order = createOrder("ord-12345", "acc-001-sandbox");
    ...
    auto req = createRequest("GET", 
                             "/api/v1/orders/ord-12345?account_id=acc-001-sandbox",
                             "/api/v1/orders/*");
    SimpleResponse res;
...
TEST_F(GetOrderHandlerTest, NotFound_Returns404) {
    EXPECT_CALL(*mockBroker_, getOrderStatus("acc-001-sandbox", "non-existent"))
        .Times(1)
        .WillOnce(Return(std::nullopt));

    auto req = createRequest("GET", 
                             "/api/v1/orders/non-existent?account_id=acc-001-sandbox",
                             "/api/v1/orders/*");
    SimpleResponse res;
...
TEST_F(GetOrderHandlerTest, MissingAccountId_Returns400) {
    auto req = createRequest("GET", 
...
TEST_F(GetOrderHandlerTest, PostMethod_Returns405) {
    auto req = createRequest("POST", "/api/v1/orders/ord-12345");
    SimpleResponse res;
...
TEST_F(GetOrderHandlerTest, DeleteMethod_Returns405) {
    auto req = createRequest("DELETE", "/api/v1/orders/ord-12345");
    SimpleResponse res;
...
TEST_F(GetOrderHandlerTest, AccountNotFound_Returns404) {
    EXPECT_CALL(*mockBroker_, getOrderStatus("unknown-account", "ord-12345"))
        .Times(1)
        .WillOnce(Throw(std::runtime_error("Account not found")));

    auto req = createRequest("GET", 
                             "/api/v1/orders/ord-12345?account_id=unknown-account",
                             "/api/v1/orders/*");
    SimpleResponse res;
