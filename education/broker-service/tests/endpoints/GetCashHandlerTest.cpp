 ...
 * 
 * GET /api/v1/portfolio/cash?account_id=xxx — только кэш
 */
...
protected:
    void SetUp() override {
        ...
        handler_ = std::make_unique<GetCashHandler>(mockBroker_);
    }
    ...
    domain::Portfolio createTestPortfolio(const std::string& accountId, 
                                          double cash,
                                          const std::string& currency = "RUB") {
        domain::Portfolio p;
        p.accountId = accountId;
        p.cash = domain::Money::fromDouble(cash, currency);
        p.totalValue = p.cash;
        return p;
    }
...
TEST_F(GetCashHandlerTest, ReturnsUnitsFormat) {
...
TEST_F(GetCashHandlerTest, DifferentCurrency_ReturnsCorrectCurrency) {
    ...
    auto json = parseJson(res.getBody());
    EXPECT_EQ(json["units"], 5000);
    EXPECT_EQ(json["currency"], "USD");
}

TEST_F(GetCashHandlerTest, ZeroCash_ReturnsZero) {
    auto portfolio = createTestPortfolio("acc-001-sandbox", 0.0);
    ...
    auto json = parseJson(res.getBody());
    EXPECT_EQ(json["units"], 0);
}

TEST_F(GetCashHandlerTest, AccountNotFound_Returns404) {
    EXPECT_CALL(*mockBroker_, getPortfolio("unknown-account"))
        .Times(1)
        .WillOnce(Throw(std::runtime_error("Account not found")));

    auto req = createRequest("GET", "/api/v1/portfolio/cash?account_id=unknown-account");
    SimpleResponse res;
...
TEST_F(GetCashHandlerTest, MissingAccountId_Returns400) {
...
TEST_F(GetCashHandlerTest, PostMethod_Returns405) {
    auto req = createRequest("POST", "/api/v1/portfolio/cash?account_id=acc-001-sandbox");
    SimpleResponse res;
...
TEST_F(GetCashHandlerTest, PutMethod_Returns405) {
    auto req = createRequest("PUT", "/api/v1/portfolio/cash?account_id=acc-001-sandbox");
    SimpleResponse res;
