 ...
 * 
 * Архитектура:
 * - Exchange: topic (broker.events)
 * - Routing keys: order.placed, order.executed, order.rejected, etc.
 ...
 * 
 * // Подписка
 * adapter->subscribe({"order.placed"}, [](const std::string& key, const std::string& msg) {
 ...
 * 
 * // Публикация
 * adapter->publish("order.executed", R"({"order_id":"ord-001"})");
 * ```
 */
